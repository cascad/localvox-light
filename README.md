...
после `npm run build` + перезагрузки страницы, БЕЗ пересборки Rust; правку Rust — обычным `cargo build`.

Грабли (все — реальные из этой работы):

- **Повар — отдельный бинарь.** `cargo build -p localvox-light` НЕ собирает `localvox-process`.
  Бери `--workspace` (или добавляй `-p localvox-light-asr`) — иначе расшифровка и сводка идут старым
  поваром (например, без выбора провайдера сводки).
